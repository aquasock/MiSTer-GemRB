...
| `MISTER_CPUS=0-1` | ARM cores the game runs on, as a `taskset` list such as `0` or `0-1`. The default lets the game's audio and helper threads use both cores; `0` keeps every thread on CPU 0 |
| `MISTER_MAIN_CPUS=0` | Cores for the game's main thread, within `MISTER_CPUS`. The MiSTer frontend occupies CPU 1, so the default is CPU 0 |
